...
import com.arch.experiments.tests.test_049.lib_core.*

// TODO
class EventMachineFactory<T>(
    private val onObserve: (T) -> Unit = { },
    private val onPush: (push: (T) -> Unit) -> Unit = { }
) {
    ...
    private fun createLinkedMachine(initialState: T): Machine<T> {
        ...
        val linker = MachineLinker(false, initialState)
        val presenterMachine = linker.createLinkedMachine()
        val viewMachine = linker.createLinkedMachine()
