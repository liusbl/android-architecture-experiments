...
import com.arch.experiments.tests.test_051.lib_core.Observer
...
fun TextView.createTextPusherFactory(): Pusher.Factory<String> =
    StateHandlerFactory<String>(onObserve = ::setText).createPusherFactory()

fun View.createVisibilityPusher(initialState: Boolean) =
    StateHandlerFactory(onObserve = ::isVisible.setter).createPusher(initialState)
...
fun View.createClickObserver(): Observer<Unit> {
    return StateHandlerFactory<Unit>( // TODO Should be event, not state
...
//    EventMachineFactory(onObserve = ::showToast).createPusher("")
