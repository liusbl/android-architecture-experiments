...
import com.arch.experiments.tests.test_038.lib.BaseViewModel
import com.arch.experiments.tests.test_038.lib.ToggleAction
import com.arch.experiments.tests.test_038.lib.ToggleEffect
import com.arch.experiments.tests.test_038.misc.MyPreferences
import io.reactivex.Observable
import java.util.concurrent.TimeUnit

class Test38ViewModel(
        ...
        onAction(
            ...
            Observable.just {
                state.withEffect { copy(toastEffect = it) }
                    .withEffect { copy(popupEffect = it) }
            }
        )
    }
    ...
    private fun actionsToStates() {
        onAction(State::changeStateInstantlyAction) {
            state.copy(
                ...
                if (state.changeStateInstantlyButtonColor == R.color.red) R.color.blue else R.color.red
            )
        }
        ...
        onAction(
            ...
            Observable.just({
                state.copy(
                    ...
                    if (state.changeStateAsyncButtonColor == R.color.green) R.color.yellow else R.color.green
                )
            })
        )
    }
    ...
    private fun statesToEffects() {
        ...
        onState(
            ...
            Observable.just(state.withEffect { copy(toastEffect = it) })
        )
    }
    ...
    private fun example1_greeting() {
        ...
        onAction(
            ...
            createGreeting(state.greetName)
                ...
                .startWith(
                    // TBD: Think of which strategy is better
                    addEffect {
                        copy(
                            isGreetingLoading = true,
                            animateButtonEffect = it, // TODO Check if this works
                            showGreetStartedEffect = it // TODO Check if this works
                        )
                    }
//                    {
//                        state.copy(isGreetingLoading = true)
//                            .pushEffect { copy(animateButtonEffect = it) } // PTR: Maybe like this?
//                            .pushEffect { copy(showGreetStartedEffect = it) }
//                    }
                )
                .map { { addEffect { copy(showGreetFinishedEffect = it) } } } // TODO too many brackets
        )
    }
