...
import com.arch.experiments.tests.test_053.lib_core.*

class StateHandlerFactory<T>(
    val onObserve: (T) -> Unit = { },
    val onPush: (push: (T) -> Unit) -> Unit = { },
    val isEvent: Boolean // TODO find better (more clearConfig) way to use
) {
    ...
    private fun createLinkedMachine(initialState: T): Machine<T> {
        ...
        val viewMachine = linker.attachLinkedMachine(!isEvent)
